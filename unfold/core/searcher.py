...
class FileSearcher:
    ...
    def _calculate_string_similarity(self, query: str, target: str) -> float:
        ...
        if self.enable_fuzzy and len(query) > 2:
            ...
            lev_sim = 1 - (levenshtein_distance(query_lower, target_lower) / max(len(query_lower), len(target_lower)))
            
            # Jaro-Winkler similarity
            jaro_sim = textdistance.jaro_winkler(query_lower, target_lower)
            
            # Jaccard similarity (for word-based matching)
            jaccard_sim = textdistance.jaccard(query_lower.split(), target_lower.split())
    ...
    def _calculate_frequency_recency_score(self, access_count: int, last_accessed: Optional[float]) -> float:
        ...
        if last_accessed:
            ...
            recency_score = max(0, 10 - (hours_since_access / 24) * self.weights['recency_factor'])
        
        return frequency_score + recency_score
    
    def _get_match_type_and_base_score(self, query: str, name: str) -> Tuple[str, float]:
    ...
    def _apply_file_type_bonus(self, query: str, file_type: Optional[str], base_score: float) -> float:
        ...
        if file_type in code_extensions and any(char in query for char in ['_', '-', 'test', 'spec']):
            return base_score + (self.weights['file_type_bonus'] * 0.5)
        
        return base_score
    
    def _rank_results(self, results: List[Dict[str, Any]], query: str) -> List[SearchResult]:
        """Rank search results using multiple algorithms."""
        ranked_results = []
        
        for result in results:
            # Base similarity score
            match_type, base_score = self._get_match_type_and_base_score(query, result['name'])
    ...
    def search(
        ...
        if self.cache_results:
            ...
            if cached:
                return [SearchResult(result, result['score'], result['match_type']) for result in cached]
        ...
        if file_types:
            ...
            results = [r for r in results if r.get('file_type', '').lower() in file_types_lower]
